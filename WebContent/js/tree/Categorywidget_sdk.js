...
function  Categorywidget_sdk(Configs){
  ...
  this.readComboData=function(callBack){
     $.ajax({
          ...
          error:function(){
        	  alert((myself.DataType=="1"?"读取渠道分组数据出错！":"读取游戏分组数据出错！"));
          }
      });
  }
  /**
  ...
  this.CreateMoveButton=function(){
	  $("#SysChannelsSearcher"+myself.interfaceName+"Bt3").html('<button id="'+myself.selfName+myself.interfaceName+'MoveRightAll" type="button"  style="width:100px;">添加全部>></button><br/><button id="'+myself.selfName+myself.interfaceName+'MoveRight" type="button" style="width:100px;">添加>></button><br/><button id="'+myself.selfName+myself.interfaceName+'MoveLeft" type="button" style="width:100px;">删除<<</button><br/><button id="'+myself.selfName+myself.interfaceName+'MoveLeftAll" type="button" style="width:100px;">删除全部<<</button>');
		...
		$("#"+myself.selfName+myself.interfaceName+'MoveRightAll').bind('click',myself.AllChannelsright);
	    $("#"+myself.selfName+myself.interfaceName+'MoveLeftAll').bind('click',myself.AllChannelsleft);
  }
  /**
   * 创建分类选择菜单
   */
  this.CreateClassCombo=function(data){
    var setting = {
      ...
      callback: {
        onCheck: function(){myself.loadChannelsData()}
      },
      data: {
        simpleData: {
          enable: true
    ...
    myself.TreeCombo = $.fn.zTree.init($("#"+myself.interfaceName+myself.selfName+"treeBox"), setting, data);
    return;
/*    $("#SysChannelsSearcher"+myself.interfaceName+"Bt1").html('<select id="'+myself.selfName+myself.interfaceName+'ComboTree" name="pid" class="easyui-combotree" style="width:200px;"  data-options="hasDownArrow:true" multiple ></select> ');
  ...
  this.Channelsright=function(){
    if($("#"+myself.ComboId+" option:selected").length>0){
      $("#"+myself.ComboId+" option:selected").each(function(){
        $("#"+myself.ComboSaveId).append("<option value='"+$(this).val()+"'>"+$(this).text()+"</option");
        myself.ChannelsSaveDataCache[myself.ChannelsSaveDataCache.length]= $(this).val();
        $(this).remove();
        return;
      });
    }else{
      alert((myself.DataType=="1"?"请选择要添加的渠道！":"请选择要添加的游戏！"));
    }
  }
  /**
   * 响应左移动事件
    ...
    if($("#"+myself.ComboSaveId+" option:selected").length>0){
      $("#"+myself.ComboSaveId+" option:selected").each(function(){
        ...
        return;
    	 ...
    	 alert(myself.DataType=="1"?"请选择要删除的渠道！":"请选择要删除的游戏！");
    }
  }
  /**
   * 响应右移动事件
   */
  this.AllChannelsright=function(){

	myself.SelectAll("Combo");
	myself.Channelsright();
  }
  /**
   * 响应左移动事件
   */
  this.AllChannelsleft=function(){

	myself.SelectAll("SaveCombo");
	myself.Channelsleft();
  }
  /**
   * 读取渠道数据
  ...
  this.loadChannelsData=function(){
    ...
    //var Values = $('#'+myself.selfName+myself.interfaceName+'ComboTree').combotree('getValues');
      $.ajax({
          ...
          error:function(){
        	  alert((myself.DataType=="1"?"读取渠道数据出错！":"读取游戏数据出错！"));
          }
      });
  }
  /**
  ...
  this.WriteDataToCombo=function(datas){
    ...
    for (var i=0; i<datas.length;i++) {
      ...
      if (jQuery.inArray(value, myself.ChannelsSaveDataCache) == -1&&jQuery.inArray(value, saveData) == -1){
    	  $("#"+myself.ComboId).append("<option value='"+value+"'>"+text+"</option"); 
      }
      saveData[saveData.length]=value;
    }
  ...
  this.CreateLeftCombo=function(){
    $("#SysChannelsSearcher"+myself.interfaceName+"Bt2").html('<input type="text" id="'+myself.ComboId+'FilterInput" onkeyup="$.fun.select_filter(this.value,\''+myself.ComboId+'\');" style="width:197px;color:#888888" value="<-- 搜索 -->" name=""><br><select id="'+myself.ComboId+'"  style="text-align:left;width:200px;height:150px;" multiple="multiple" name="'+myself.ComboName+'"></select>');
